...
- Runs: `pytest tests/test_manual/ -v --html=reports/manual_tests_<timestamp>.html --self-contained-html` (timestamp matches `logs/manual_<timestamp>.log` for that run).
- Extra arguments are forwarded to pytest (e.g. `-m nullcde`, `-k test_name`).
- Output: standalone **pytest-html** report at `reports/manual_tests_<timestamp>.html` (one per run; not overwritten). This is separate from the framework CLI’s `report_*.html` (from `python -m sts_test_framework.cli`). The project depends on `pytest-html` for this path.
...
| Goal                                                                | Open this                                                                                                                                       |
| ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Run **all three** report pipelines in one command                   | `bash scripts/run_full_suite.sh` — then open the artifacts below (`manual_tests_*.html`, per-model `report_*.html`, term-verify `*.md` / `*.csv`) |
...
| Manual pytest results as HTML                                       | Newest `reports/manual_tests_*.html` from `bash scripts/run_manual_tests.sh`                                                                    |
