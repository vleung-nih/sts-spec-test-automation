...
This repo includes a ready-to-use workflow in `.github/workflows/sts-tests.yml` that runs on push and pull request to `main`/`master`. It uses **STS QA** (`https://sts-qa.cancer.gov/v2`) by default; override via the repository variable `STS_BASE_URL` if needed. Reports are uploaded as artifacts.
